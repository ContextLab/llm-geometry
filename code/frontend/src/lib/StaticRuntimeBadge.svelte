<script lang="ts">
  ...
  // idle → loading → webgpu·q4f16 / wasm·q8 → (or error). Polled — the runtime
  ...
  const text = $derived(
    status === "ready" && info?.device && info?.dtype
      ? `in-browser · ${info.device} · ${info.dtype}`
      : status === "loading"
  ...
  const tip = $derived(
    status === "error"
      ...
      : status === "ready"
        ? `generation runs locally via transformers.js (${info?.onnx_repo ?? "ONNX export"}) — real logits, no server`
